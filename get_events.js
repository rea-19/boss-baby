...
function iterateRecords(data) {
    ...
    // variable declaration of the following values: event name, location, date, age, and image
    Object.values(records).forEach(value => {
        ...
        // Filter out events for young adults, adults, seniors that are irrelevant to our target audience
        ...
        if (rawCost) {
            // if rawCost is "free", price is 0
            // rawCost.replace(/[^0-9]/g, '') removes all non-numeric characters from the string
            // parseInt(..., 10) converts string into an integer
            //  ||0 if parseInt returns no digit, the price defaults to 0
            let price = rawCost.toLowerCase() === "free" ? 0 : parseInt(rawCost.replace(/[^0-9]/g, ''), 10) || 0;

            // points allocation for the events
            if (price === 0) points = 1;
            else if (price <= 10) points = 2;
            else if (price > 10) points = 3;

            // price labels on browse events are based on the price ranges
